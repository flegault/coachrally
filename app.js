...
function blankMatch(){return{id:uid(),teamId:null,schemaVersion:5,status:'draft',team:'',opp:'',date:'',time:'',place:'',fanMessage:'',side:'visiteur',fixed:true,rotateSixBatting:true,battingRotation:{nextId:null,pending:[],order:[]},innings:4,players:[],order:[],battingOrders:{},schedule:[],started:false,locks:{innings:{},halves:{}},cloud:{matchId:null,publicId:null,publicPassword:''},createdAtMs:Date.now(),updatedAtMs:Date.now()}}
...
function defaults(){let m=blankMatch();return{schemaVersion:5,teams:[],activeTeamId:null,matches:[],activeMatchId:null,route:'accueil',teamProfile:{name:'',publicId:null,publicSlug:'',publicPassword:'',fanMessage:'Merci de nous encourager!',roster:[]},roster:[],team:m.team,opp:m.opp,date:m.date,time:m.time,place:m.place,fanMessage:m.fanMessage,side:m.side,fixed:m.fixed,rotateSixBatting:m.rotateSixBatting,battingRotation:m.battingRotation,innings:m.innings,players:m.players,order:m.order,battingOrders:m.battingOrders,schedule:m.schedule,started:m.started,locks:m.locks,cloud:m.cloud}}
function normalizeArchive(raw){
  ...
  if(raw.schemaVersion===1&&raw.match&&Array.isArray(raw.players)){
    ...
    let battingRotation=raw.battingRotation&&typeof raw.battingRotation==='object'?raw.battingRotation:{};battingRotation={nextId:idSet.has(String(battingRotation.nextId||''))?String(battingRotation.nextId):null,pending:(Array.isArray(battingRotation.pending)?battingRotation.pending:[]).map(String).filter((id,i,a)=>idSet.has(id)&&a.indexOf(id)===i),order:(Array.isArray(battingRotation.order)?battingRotation.order:[]).map(String).filter((id,i,a)=>idSet.has(id)&&a.indexOf(id)===i)};
    return{id:String(raw.id||uid()),schemaVersion:1,type:'match',completedAt:String(raw.completedAt||'').slice(0,40),match:{team:String(raw.match.team||'').slice(0,80),opp:String(raw.match.opp||'').slice(0,80),date:String(raw.match.date||'').slice(0,20),time:String(raw.match.time||'').slice(0,10),place:String(raw.match.place||'').slice(0,120),fanMessage:cleanFanMessage(raw.match.fanMessage),side:raw.match.side==='locale'?'locale':'visiteur'},fixed:raw.fixed!==false,rotateSixBatting:raw.rotateSixBatting!==false,battingRotation,innings,players,order,schedule,battingOrders,locks}
  }
...
function normalizeMatch(raw,roster){
  ...
  m.rotateSixBatting=m.rotateSixBatting!==false;
  ...
  m.battingRotation=m.battingRotation&&typeof m.battingRotation==='object'?m.battingRotation:{};
  m.battingRotation.nextId=ids.has(String(m.battingRotation.nextId||''))?String(m.battingRotation.nextId):null;
  m.battingRotation.pending=(Array.isArray(m.battingRotation.pending)?m.battingRotation.pending:[]).map(String).filter((id,i,a)=>ids.has(id)&&a.indexOf(id)===i);
  m.battingRotation.order=(Array.isArray(m.battingRotation.order)?m.battingRotation.order:[]).map(String).filter((id,i,a)=>ids.has(id)&&a.indexOf(id)===i);
  let activeSeen=0;
...
function exposeMatch(m){
  m=m||blankMatch();
  Object.assign(state,{team:m.team,opp:m.opp,date:m.date,time:m.time,place:m.place,fanMessage:m.fanMessage,side:m.side,fixed:m.fixed,rotateSixBatting:m.rotateSixBatting,battingRotation:m.battingRotation,innings:m.innings,players:m.players,order:m.order,battingOrders:m.battingOrders,schedule:m.schedule,started:m.started,locks:m.locks,cloud:m.cloud});
}
...
function safeState(raw){
  ...
  syncStateAliases(s);
  let m=s.matches.find(x=>x.id===s.activeMatchId)||null;if(m)Object.assign(s,{team:m.team,opp:m.opp,date:m.date,time:m.time,place:m.place,fanMessage:m.fanMessage,side:m.side,fixed:m.fixed,rotateSixBatting:m.rotateSixBatting,battingRotation:m.battingRotation,innings:m.innings,players:m.players,order:m.order,battingOrders:m.battingOrders,schedule:m.schedule,started:m.started,locks:m.locks,cloud:m.cloud});
...
function applyPrivateTeamPayload(team,payload,remoteMs){if(!team||!payload)return;team.name=String(payload.name||'').trim().slice(0,80);team.fanMessage=cleanFanMessage(payload.fanMessage);team.roster=normalizePlayers(payload.roster);team.roster.forEach(p=>syncRosterPlayerToUnstartedMatches(team.id,p));team.publicId=payload.publicId?String(payload.publicId).slice(0,100):null;team.publicSlug=normalizeTeamPublicId(payload.publicSlug||payload.publicId||'');team.cloud={enabled:true};team.updatedAtMs=Number(remoteMs)||Date.now()}
...
async function refreshCloudMatches(silent){if(!cloud.user){renderMatchTables();renderMini();renderHome();return}try{cloud.matchesLoading=true;renderMatchTables();let mod=await cloudModule();cloud.matches=await mod.listMatches();cloud.matchesLoaded=true}catch(e){if(!silent)modal('Liste impossible',e.message||String(e));cloud.matches=[];cloud.matchesLoaded=true}finally{cloud.matchesLoading=false;renderMatchTables();renderMini();renderHome();renderInputs()}}
...
async function openBenchRoute(id){
  ...
  if(id==='local'){
    if(!activeMatch()){renderBenchError('Aucun match actif','Prépare ou ouvre un match pour afficher l’aperçu local du banc.');return}
    cloud.publicView=publicProjection();renderYoungBench('local');return
  }
...
function benchPlayerName(row){return '<b>'+esc(row?.name||'—')+'</b>'}
function benchActivePlayerName(row){return '<span class="benchActiveName"><b>'+esc(row?.name||'—')+'</b></span>'}
function benchBatting(rows,compact){return '<div class="benchBatting '+(compact?'compact':'')+'">'+rows.map(row=>'<div class="benchBatter"><strong>'+esc(row.rank)+'</strong>'+benchActivePlayerName(row)+'</div>').join('')+'</div>'}
function benchDefense(rows,compact){let byPosition={};rows.forEach(row=>byPosition[row.position]=row);let positions=[['L1','Lanceur gauche'],['L2','Lanceur droite'],['AC','Arrêt-court'],['1B','Premier but'],['2B','Deuxième but'],['3B','Troisième but']];return '<div class="benchDefense '+(compact?'compact':'')+'">'+positions.map(([code,label])=>{let row=byPosition[code]||{name:'—'},cap=code==='L1'||code==='L2'?' 🧢':'';return '<div class="benchDefender">'+benchActivePlayerName(row)+'<span class="benchDefenseRole"><strong>'+esc(code)+' — '+esc(label)+cap+'</strong></span></div>'}).join('')+'</div>'}
function benchPhase(phase,compact){if(!phase)return'';let body=phase.type==='attaque'?benchBatting(phase.rows,compact):benchDefense(phase.rows,compact),icon=phase.type==='attaque'?'🏏':'🧤';return '<div class="benchPhaseHead"><span><span aria-hidden="true">'+icon+'</span> '+esc(phase.label)+'</span></div>'+body}
...
function renderYoungBench(connection){
  let box=$('#youngBench'),model=window.RallyeCapBench.buildModel(cloud.publicView),local=connection==='local',sync=local?'Aperçu local':connection==='offline'?'Hors ligne':'Synchronisé',syncClass=local?'local':connection==='offline'?'offline':'online';
  ...
  let next=model.next?benchPhase(model.next,true):'<div class="benchLast"><span>🎉</span><strong>Dernière manche</strong><p>Encourage ton équipe!</p></div>';
  box.innerHTML=header+'<main class="benchDashboard"><section class="benchNow"><h1>Maintenant</h1>'+benchPhase(model.current,false)+'</section><aside class="benchNext"><h2>Ensuite</h2>'+next+'</aside></main>';
}
...
function confirmWarnings(warnings,onYes){
  modal('Avertissements à vérifier','',[{label:'Annuler',kind:'secondary'},{label:'Confirmer',kind:'danger',onClick:onYes}]);
  let root=$('#modalText'),list=document.createElement('ul'),question=document.createElement('p');
  list.className='modalWarningList';warnings.forEach(text=>{let item=document.createElement('li');item.textContent=text;list.appendChild(item)});
  question.textContent='Veux-tu tout de même commencer le match?';root.replaceChildren(list,question);root.classList.remove('hide')
}
...
function snapshotBattingOrder(i,half){if(!isBattingHalf(half))return;let result=battingResult(i);state.battingOrders[phaseKey(i,half)]=result.ids.slice();state.battingRotation={nextId:result.nextId,pending:result.pending.slice(),order:result.order.slice()}}
...
function setHalfLock(i,half,locked){let other=half==='debut'?'fin':'debut';if(isInningLocked(i)){delete state.locks.innings[i];state.locks.halves[phaseKey(i,other)]=true}if(locked){snapshotBattingOrder(i,half);state.locks.halves[phaseKey(i,half)]=true}else{delete state.locks.halves[phaseKey(i,half)];clearBattingSnapshot(i,half)}}
function resetMatch(target){state.started=false;state.locks={innings:{},halves:{}};state.battingOrders={};state.battingRotation={nextId:null,pending:[],order:[]};matchIndex=0;optimizeDirty=true;if(target){state.route=normalizeRoute(String(target).replace('#',''));location.hash='#'+state.route}save();renderAll()}
...
function markReadyToPlay(){
  ...
  let warnings=analyze().validations.filter(v=>!v.ok&&!v.nonBlocking);
  let commit=()=>{let m=activeMatch();if(!m)return;m.status='ready';save();location.hash='#jouer';renderAll()};
  if(warnings.length){confirmWarnings(warnings.map(v=>v.text),commit);return}
  commit()
}
...
function beginPlay(){if(isReadOnlyMatch()||state.route!=='jouer'||!isMatchReady())return;state.started=true;let order=active().map(p=>p.id);state.battingRotation={nextId:order[0]||null,pending:[],order};let m=activeMatch();if(m)m.status='active';save();renderAll()}
...
function isPlayerPresent(p){return p?.on!==false}
function active(){let ids=new Set(state.players.filter(isPlayerPresent).map(p=>p.id));let arr=state.order.filter(id=>ids.has(id)).map(player).filter(Boolean);state.players.filter(p=>isPlayerPresent(p)&&!state.order.includes(p.id)).forEach(p=>arr.push(p));return arr.slice(0,MAX)}
...
function tablePlayers(){let hist=historicalIds();if(!isMatchStarted())return active();let activeIds=new Set(state.players.filter(isPlayerPresent).map(p=>p.id)),seen=new Set(),rows=[];state.order.forEach(id=>{let p=player(id);if(p&&(activeIds.has(id)||hist.has(id))&&!seen.has(id)){rows.push(p);seen.add(id)}});state.players.forEach(p=>{if((activeIds.has(p.id)||hist.has(p.id))&&!seen.has(p.id)){rows.push(p);seen.add(p.id)}});return rows.slice(0,MAX+12)}
...
function battingResult(inning){
  let order=active().map(p=>p.id),half=state.side==='visiteur'?'debut':'fin',snap=state.battingOrders?.[phaseKey(inning,half)];
  if(Array.isArray(snap))return{ids:snap.slice(),nextId:state.battingRotation?.nextId||null,pending:(state.battingRotation?.pending||[]).slice(),order:(state.battingRotation?.order||order).slice()};
  let nextId=isMatchStarted()?state.battingRotation?.nextId:order[0],pending=isMatchStarted()?(state.battingRotation?.pending||[]):[],rotationOrder=isMatchStarted()?(state.battingRotation?.order||order):order,first=0;
  if(isMatchStarted()){while(first<state.innings&&isHalfLocked(first,half))first++}
  let result={ids:[],nextId,pending:pending.slice(),order:rotationOrder.slice()};
  for(let i=first;i<=inning;i++)result=RallyeCapBattingOrder.buildInning({order,rotationOrder:result.order,nextId:result.nextId,pending:result.pending,rotateSix:state.rotateSixBatting});
  return result
}
function batters(inning){let all=new Map(state.players.map(p=>[p.id,p]));if(!all.size||!state.fixed)return[];return battingResult(inning).ids.map((id,j)=>{let p=all.get(id);return p?{id:p.id,rank:j+1,name:p.name,number:p.number||'',label:playerLabel(p),global:active().findIndex(x=>x.id===id)+1}:null}).filter(Boolean)}
...
function lineupEngineOptions(){return{playerIds:active().map(p=>p.id),innings:state.innings,fixed:state.fixed,batterIdsByInning:Array.from({length:state.innings},(_,i)=>batters(i).map(b=>b.id)),rules:RallyeCapRules}}
function optimizeLineup(){invalidateReady();state.battingOrders={};state.battingRotation={nextId:null,pending:[],order:[]};state.schedule=RallyeCapLineupEngine.generateSchedule(lineupEngineOptions());optimizeDirty=false;lineupAutoDirty=false;analysis=null}
...
function generateInning(i,prior){return RallyeCapLineupEngine.generateInning(Object.assign(lineupEngineOptions(),{inning:i,priorSchedule:prior}))}
...
function analyze(){ensureSchedule();problems=new Map();let vals=[],sugs=[],ids=active().map(p=>p.id);let st=collectStats(),summary=RallyeCapRules.scheduleRuleSummary(state.schedule,ids);let mark=(id,i,msg)=>{let k=id+':'+i;if(!problems.has(k))problems.set(k,[]);problems.get(k).push(msg)};let pname=id=>player(id)?.name||'Joueur';
  ...
  let firstBaseRepeatRequired=state.innings>ids.length;if(firstBaseRepeatRequired){vals.push({ok:false,warn:true,nonBlocking:true,text:'Rotation au premier but: une répétition est nécessaire avec '+state.innings+' manches et '+ids.length+' joueurs.'})}else{summary.firstBase.forEach(x=>x.ins.forEach(i=>mark(x.id,i,'Premier but plus d’une fois')));vals.push({ok:!summary.firstBase.length,text:!summary.firstBase.length?'Chaque joueur est 1B au maximum une fois.':'Un joueur joue 1B plus d’une fois.'});summary.firstBase.slice(0,5).forEach(x=>sugs.push(makeFirstBaseSuggestion(x.id,x.ins)))};
...
function renderInputs(){
  ...
  let rotate=$('#rotateSixBatting'),rotateField=$('#rotateSixBattingField');if(rotate){rotate.checked=state.rotateSixBatting;rotate.disabled=prepLocked||!state.fixed}if(rotateField)rotateField.classList.toggle('inactive',!state.fixed)
  ...
  let listNew=$('#newMatchFromListBtn');if(listNew){listNew.classList.toggle('hide',!!workflowMatch()||resumableCloudMatchRows().length>0||!hasTeamSetup());listNew.disabled=!hasTeamSetup()}
...
function resumableCloudMatchRows(){if(workflowMatch())return[];return combinedMatchRows('active').filter(r=>!r.local&&!['completed','archived'].includes(r.match.status))}
...
function renderHome(){
  ...
  let m=workflowMatch(),teamName=teamProfileName(),remoteRows=resumableCloudMatchRows();
  if(!m){
    if(remoteRows.length===1){let row=remoteRows[0],remote=row.match,meta=[remote.opp?'vs '+remote.opp:'Adversaire à définir',remote.date?formatDate(remote.date):'',remote.time||'',remote.place||''].filter(Boolean).join(' • ');box.innerHTML='<div><h2>Match en ligne à reprendre</h2><p class="hint">'+esc(meta||teamName)+' • '+esc(matchStatusLabel(remote))+'</p></div><div class="homeActions"><button class="btn brandBtn" id="homeResumeCloudMatch" type="button">Reprendre le match</button></div>';let resume=$('#homeResumeCloudMatch');if(resume)resume.onclick=()=>openCombinedMatch(row.key);return}
    if(remoteRows.length>1){box.innerHTML='<div><h2>Matchs en ligne à reprendre</h2><p class="hint">'+remoteRows.length+' matchs non commencés ou en cours sont disponibles pour '+esc(teamName)+'.</p></div><div class="homeActions"><a class="btn brandBtn" href="#matchs">Voir les matchs</a></div>';return}
...
function isUnstartedTeamMatch(m,teamId){return !!m&&m.teamId===teamId&&!m.started&&(m.status==='draft'||m.status==='ready')}
function syncRosterPlayerToUnstartedMatches(teamId,rosterPlayer){let changed=[];state.matches.forEach(m=>{if(!isUnstartedTeamMatch(m,teamId))return;let p=m.players.find(x=>x.id===rosterPlayer.id),dirty=false;if(!p){p={id:rosterPlayer.id,name:rosterPlayer.name,number:rosterPlayer.number||'',on:false};m.players.push(p);if(!m.order.includes(p.id))m.order.push(p.id);dirty=true}else{if(p.name!==rosterPlayer.name){p.name=rosterPlayer.name;dirty=true}let number=rosterPlayer.number||'';if((p.number||'')!==number){p.number=number;dirty=true}}if(dirty){m.updatedAtMs=Date.now();changed.push(m)}});return changed}
function syncUnstartedMatchesCloud(matches){if(!cloud.user)return;let activeId=activeMatch()?.id;Array.from(new Set(matches)).filter(m=>m.id!==activeId&&m.cloud?.matchId).forEach(m=>saveLocalMatchCloud(m).catch(()=>{}))}
function addRosterPlayers(entries){let t=ensureActiveTeam(),existing=new Set(state.roster.map(p=>p.name.toLowerCase())),changed=[];entries.forEach(entry=>{let name=typeof entry==='string'?entry:entry.name,number=typeof entry==='string'?'':cleanNumber(entry.number),key=name.toLowerCase();if(existing.has(key))return;existing.add(key);let p={id:uid(),name,number,on:true};state.roster.push(p);changed.push(...syncRosterPlayerToUnstartedMatches(t.id,p))});t.updatedAtMs=Date.now();state.team=t.name;save();syncUnstartedMatchesCloud(changed);syncPublicTeamIfActive();renderAll()}
...
function addRosterPlayersToDraftMatch(entries){let current=activeMatch();if(!current||isMatchStarted()||isReadOnlyMatch())return;let t=activeTeam(),rosterByName=new Map(state.roster.map(p=>[p.name.toLowerCase(),p])),present=active().length,changed=[],playerChanged=false;entries.forEach(entry=>{let name=typeof entry==='string'?entry:entry.name,number=typeof entry==='string'?'':cleanNumber(entry.number),key=name.toLowerCase(),rp=rosterByName.get(key);if(!rp){rp={id:uid(),name,number,on:true};state.roster.push(rp);rosterByName.set(key,rp)}else if(number&&!rp.number)rp.number=number;changed.push(...syncRosterPlayerToUnstartedMatches(t.id,rp));let mp=current.players.find(p=>p.id===rp.id);if(mp&&mp.on===false&&present<MAX){mp.on=true;present++;playerChanged=true;if(!changed.includes(current))changed.push(current)}});if(t)t.updatedAtMs=Date.now();state.team=state.teamProfile.name;if(playerChanged)markPlayerDirty();save();syncUnstartedMatchesCloud(changed);syncPublicTeamIfActive();renderAll()}
...
function queuePriorityBatter(id){if(!isMatchStarted())return;let pending=state.battingRotation?.pending||[];if(!pending.includes(id))pending.push(id);state.battingRotation={nextId:state.battingRotation?.nextId||active()[0]?.id||null,pending,order:(state.battingRotation?.order||active().map(p=>p.id)).slice()}}
function removeFromBattingRotation(id){state.battingRotation=RallyeCapBattingOrder.removePlayer(state.battingRotation,id,active().map(p=>p.id))}
function addPlayers(names){let existing=new Set(state.players.map(p=>p.name.toLowerCase()));let list=names.filter(n=>!existing.has(n.toLowerCase())&&existing.add(n.toLowerCase()));if(!isMatchStarted()){let count=active().length;list.forEach(n=>{let on=count<MAX;if(on)count++;addPlayerRecord(n,on)});if(list.length)markPlayerDirty();save();renderAll();return}let skipped=[],added=0;list.forEach(n=>{if(active().length>=MAX){skipped.push(n);return}let pl=addPlayerRecord(n,true);queuePriorityBatter(pl.id);added++});if(added)markDirty();save();renderAll();if(skipped.length)modal('Maximum atteint','Il y a déjà 12 joueurs actifs. Les joueurs suivants n’ont pas été ajoutés: '+skipped.join(', ')+'. Utilise Remplacer si nécessaire.')}
...
function promptReplacePlayer(pl){let inactive=state.players.filter(p=>!isPlayerPresent(p)&&p.id!==pl.id),buttons=inactive.map(r=>({label:playerChoiceLabel(r),kind:'playerChoice',onClick:()=>replaceWithExistingPlayer(pl,r)}));buttons.push({kind:'inputReplace',onClick:name=>replaceWithNewPlayerName(pl,name)},{label:'Annuler',kind:'modalCancel'});modal('Remplacer '+pl.name,inactive.length?'Choisis un joueur inactif ou entre un nouveau nom.':'Entre un nouveau nom pour le joueur remplaçant.',buttons)}
function playerChoiceLabel(pl){return pl.name+(isPlayerPresent(pl)?'':' · inactif')}
...
function togglePlayerAvailability(pl){
  if(!isMatchStarted()){
    if(!isPlayerPresent(pl)&&active().length>=MAX){modal('Maximum atteint','Il peut y avoir au maximum 12 joueurs présents. Désactive un joueur avant d’activer '+pl.name+'.');return}
    pl.on=!isPlayerPresent(pl);markPlayerDirty();save();renderAll();return
  }
  if(!isPlayerPresent(pl)){
    ...
    confirmModal('Activer '+pl.name,'Ce joueur deviendra disponible pour les manches non barrées. Les assignations restent manuelles.',()=>{pl.on=true;queuePriorityBatter(pl.id);markDirty();save();renderAll()});return
  }
  let act=active();if(act.length<=MIN){let inactive=state.players.filter(p=>!isPlayerPresent(p));if(!inactive.length){modal('Remplacement requis','Il reste seulement 6 joueurs actifs. Ajoute ou active un remplaçant avant de retirer '+pl.name+'.');return}let buttons=inactive.map(r=>({label:r.name,kind:'secondary',onClick:()=>{r.on=true;replacePlayerForUnlocked(pl.id,r.id);pl.on=false;markDirty();save();renderAll()}}));buttons.unshift({label:'Annuler',kind:'secondary'});modal('Remplacer '+pl.name,'Il reste seulement 6 joueurs actifs. Choisis un joueur inactif pour le remplacer dans les manches non barrées.',buttons);return}confirmModal('Retirer '+pl.name,'Les manches barrées ne seront pas modifiées. Les assignations futures de ce joueur seront retirées et devront être corrigées manuellement.',()=>{clearPlayerFromUnlocked(pl.id);pl.on=false;removeFromBattingRotation(pl.id);markDirty();save();renderAll()})}
...
function renderTable(target){
  ...
  h+='</tr></thead><tbody>';
  a.forEach(pl=>{
    let inactive=!isPlayerPresent(pl),rowSel=selectedRow===pl.id;
...
function bind(){
  ...
  window.addEventListener('online',()=>{if(state.route==='banc'&&cloud.publicView)renderYoungBench(cloud.publicIdRoute==='local'?'local':'connected')});
  window.addEventListener('offline',()=>{if(state.route==='banc'&&cloud.publicView)renderYoungBench(cloud.publicIdRoute==='local'?'local':'offline')});
  ...
  ['opp','date','timeHour','timeMinute','place','fanMessage','innings','fixed','rotateSixBatting'].forEach(id=>{let el=$('#'+id);if(el)el.addEventListener('input',()=>{updateFromInputs();renderAll()})});
