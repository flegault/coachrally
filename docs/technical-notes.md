...
Le bassin permanent et les joueurs d'un match partagent des identifiants stables. Lorsqu'un joueur est ajouté ou que son nom ou son numéro change, tous les matchs `draft` ou `ready` non commencés de la même équipe sont mis à jour. Un joueur nouvellement copié dans un match est ajouté à la fin de `order` avec `on: false`; cette mutation ne modifie donc ni la défensive ni un état `ready` existant. Le passage ultérieur de `Absent` à `Présent` utilise le flux normal de changement de présence et déclenche l'optimisation automatique à l'entrée dans `Alignement`. Les matchs `active`, `completed` et `archived` ne sont jamais modifiés par cette synchronisation. Les copies cloud privées non actives sont sauvegardées explicitement quand elles possèdent déjà un `cloud.matchId`.

`Matchs` est la vue centrale. Elle affiche un tableau triable unique qui combine les matchs locaux et les matchs cloud du compte connecté, incluant les archives. Les colonnes sont `Adversaire`, `Date / heure`, `Endroit`, `Statut`, `Modifié` et `Actions`. Les doublons sont fusionnés par `cloud.matchId`. La fin de `refreshCloudMatches()` rerend aussi le compteur et la carte contextuelle de l'accueil; elle ne peut pas appeler `renderAll()` puisque `route()` relancerait le chargement sur `#matchs`. Un match seulement en ligne est importé localement quand l'utilisateur clique sa ligne ou l'action `Reprendre le match` de l'accueil. Un match cloud unique peut donc être proposé sans être importé silencieusement.
